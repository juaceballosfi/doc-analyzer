...
| `502`  | Fallo en la llamada al LLM o respuesta no válida                              |
...
| -------------- | -------------------------------------- | ------------------------------------------------------------ |
| `BASE_PATH`    | _(vacío)_                              | Prefijo de ruta inyectado por el proxy inverso (ChurroStack) |
| `LLM_BASE_URL` | `https://apps.fi-group.com/api/openai` | URL base de la API compatible con OpenAI                     |
| `LLM_API_KEY`  | _(requerido)_                          | Clave de autenticación de la API                             |
...
| `python-multipart` | Soporte de `multipart/form-data` para la subida de archivos |
